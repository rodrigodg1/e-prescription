...
class BbsBlsSignatureProof2020 extends jsonld_signatures_1.suites.LinkedDataProof {
    ...
    async deriveProof(options) {
        ...
        if (this.proof) {
            ...
            derivedProof = await jsonld_1.default.compact(this.proof, jsonld_signatures_1.SECURITY_CONTEXT_URL, {
                documentLoader,
                expansionMap,
            ...
            derivedProof = { "@context": jsonld_signatures_1.SECURITY_CONTEXT_URL };
        }
        ...
        const compactInputProofDocument = await jsonld_1.default.fromRDF(transformedInputDocumentStatements.join("\n"));
        // Frame the result to create the reveal document result
        const revealDocumentResult = await jsonld_1.default.frame(compactInputProofDocument, revealDocument, { documentLoader });
        ...
        const documentRevealIndicies = revealDocumentStatements.map(key => transformedInputDocumentStatements.indexOf(key) +
        ...
        if (!nonce) {
            nonce = await random_1.randomBytes(50);
        }
        ...
        // Compute the proof
        const outputProof = await bbs_signatures_1.blsCreateProof({
    ...
    async verifyProof(options) {
        ...
        try {
            ...
            const key = await this.LDKeyClass.from(verificationMethod);
            // Verify the proof
            const verified = await bbs_signatures_1.blsVerifyProof({
     ...
     * @returns {Promise<{string[]>}.
     */
    async createVerifyProofData(proof, { documentLoader, expansionMap }) {
     ...
     * @returns {Promise<{string[]>}.
     */
    async createVerifyDocumentData(document, { documentLoader, expansionMap }) {
    ...
    async getVerificationMethod({ proof, documentLoader }) {
        ...
        const result = await jsonld_1.default.frame(verificationMethod, {
            "@context": jsonld_signatures_1.SECURITY_CONTEXT_URL,
            "@embed": "@always",
            id: verificationMethod
        }, {
            documentLoader,
            compactToRelative: false,
            expandContext: jsonld_signatures_1.SECURITY_CONTEXT_URL
        });
        if (!result) {
